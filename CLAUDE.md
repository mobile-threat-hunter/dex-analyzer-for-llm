...
* A class that is ITSELF an interface is SKIPPED. A sub-interface declares the
  ...
  examined.
* A class is a dex fact. `constructed_in` (the methods calling one of its
  constructors) separates a live component from dead code, and an empty one is proof
  of neither — a component reached only through reflection has no construction site.
* The detector COMPLEMENTS `CUSTOM_TLS_TRUST` rather than replacing it: that tag
  reports the install, this reports the decision, and one guard drives both on one
  dex so the complementarity is asserted rather than described. It is a
  DIFFERENT question, **not a strictly stronger one** — the first cut claimed the
  latter and the review refuted it by construction: a permissive
  `X509ExtendedTrustManager` installed through the curated `SSLContext#init` is
  reported by #52 and not by this. The claim is corrected in all four mirrors.
* A descriptor may be DECLARED in several loaded dexes — an ordinary multidex app,
  and the norm for a packer session where the dump and the original both carry the
  class — so rows are DEDUPLICATED by descriptor and the body read is the
  first-wins one every descriptor-keyed API resolves. **Found by reading my own
  diff, not by a reviewer**: `find_classes_implementing` answers per DECLARATION,
  so loading the committed fixture twice produced **8 rows for 4 components** and
  an MCP `count` of 8 [[a-fix-can-widen-a-crash-it-did-not-create]] one axis over —
  a wrong ANSWER on the surface an LLM reads.

## Measured

**a/b OFF (HEAD `42d0314`) vs ON, SAME script.** The change is PYTHON-ONLY — no C++,
so the `.so` is `b3219432ae8318f2ba1e8fd8a66fd06c` on both halves and build identity
is about the SOURCE: the OFF half is `git checkout HEAD -- src/dexllm` plus removal
of the untracked new module, verified by `hasattr(dexllm, "detect_permissive_tls")
is False` before capturing and by a sha256 manifest of all 24 files matching exactly
after restoring.

44 sources — the whole bundled corpus (34) plus every committed fixture — x up to 19
axes (both verify verdicts, `identify`, load, `dex_count`, class list, a smali
digest, a whole-decompile digest, the full capability report incl. `by_caller` and
every usage's caller set, IOCs, content providers, permission callers, external
refs, the MCP tool-name list, the MCP provider payload, `dexllm.__all__`,
`sdk.__all__`) = **761 ON / 683 OFF axis records; 566 identical, 117 changed, 78
new-axis**.

The 117 are **three facts recorded once per source**: `dexllm.__all__` gains
`classify_tls_method` / `detect_permissive_tls`, `sdk.__all__` gains
`TlsTrustComponent` / `TlsTrustPort`, and the MCP catalog gains one tool — with **0
REMOVALS on any of them**, which is the additivity claim stated as a measurement.
**Every behavioural axis is identical on every source.** The 78 are the new axes,
absent on OFF by construction.

**Two sources carry a component**, and only one of them is a fixture:
`InterfaceCls.dex` reports its single `LInterfaceCls;` as `permissive`, and
`tests/data/permissive-tls.dex` reports **10 rows over 8 classes, 4 of them
permissive** — the other 6 being the two that genuinely check, the three
duck-typing traps and the abstract base. A corpus-only a/b would have been byte-identical on every
behavioural axis AND silent on the mechanism; `InterfaceCls.dex` is what makes it
fire on a dex nobody wrote for this feature.

pytest **1247 passed / 24 skipped**, TRUE corpus-less (`test_apk` MOVED aside)
**841 passed / 430 skipped / 0 failed** — 45 of the 46 new cases run in the CI leg,
since every behavioural craft is on the committed fixture — narrowed to
`tests/data/multidex.apk` **1146 passed**, the guard file green narrowed to **each
of the 34 bundled samples one at a time**, ctest **29/29** (no C++ moved), sweep
**21,374-class / 180,879 method-block 0-crash 0-timeout 0-error, GATE: PASS**,
determinism 3 processes x 3 `PYTHONHASHSEED`s -> one digest, lint trio clean (CI
scope), doc fences **78** (floor ratcheted 77 -> 78),
`scripts/check_dad_boundary.sh` clean.

## The fixture, and why it had to be authored

`tests/data/permissive-tls.dex` (4,872 B, 13 classes) is the **first fixture in
this repo that was WRITTEN rather than copied**, so its source is committed beside
it (`permissive-tls.java`) and the toolchain is named in
[tests/data/README.md](tests/data/README.md) (`javac 17.0.17`, `d8 8.6.2-dev` from
build-tools 35.0.0). The corpus offers no `HostnameVerifier` implementor at all,
and — more decisive — no NEGATIVE control anywhere: without a verifier that
genuinely compares and a trust manager that genuinely throws, a build that simply
reports every implementor passes every other assertion and looks like a working
detector.

**Every class in it earns its place, and eight of the thirteen were added because
a mutant walked through the first cut.** Two permissive components; two that
check; `PermissiveBoth`, the only class yielding TWO rows, without which the
documented row sort's second component is unobservable; a verifier with a
non-constructor method and TWO constructors whose callers arrive UNSORTED, without
which the `<init>` filter, the caller dedupe and the caller sort are each a no-op;
`DuckTrust` / `ExtTrust` / `InheritTrust`, the three duck-typing traps — the last
reachable ONLY through the superclass test; and `SubVerifier` / `SubAllowAll`, the
sub-interface and its invisible implementor. Two installers construct the
permissive ones through the PLATFORM APIs, so the same dex also yields
`CUSTOM_TLS_TRUST == 4` and a `constructed_in` cross-reference — which is what
lets one test state the issue: the capability report says the app supplies its own
trust, and only the detector says which decisions accept everything.

**That is the argument for authoring a fixture rather than finding one.** Every
one of the ten surviving mutants below was closed by adding a SHAPE, not by
adding an assertion — which a copied fixture cannot offer.

d8 is not byte-reproducible across versions, so the committed bytes are the artefact
and the source is the statement of intent — which is why the guards pin the exact
rendering they depend on rather than trusting a rebuild.

## Guards

46 cases in [tests/test_permissive_tls.py](tests/test_permissive_tls.py). Every
behavioural case runs on the committed fixture, so they hold in the corpus-less CI
leg and under any `$DEXLLM_TEST_APK` narrowing; the one corpus case is the evidence
from `InterfaceCls.dex` and goes through `require_corpus_shape`, so a narrowing
SKIPS where a bundled zero would FAIL (issue #46).

The predicate is exercised on BOTH layers deliberately. `classify_tls_method` is
pure — it takes the smali TEXT, not a `DexKit` — so the shapes no dex in reach
carries (a `const/16`, a returned register that is not the loaded one, an unreadable
rendering, a method with no instructions) are testable without a dex, and the
end-to-end cases pin that the pure predicate is what the DexKit path consults. The
two-entry component table is pinned as a LITERAL: a guard parametrised over the
table cannot catch an EDIT of the table, and retargeting the detector at an
interface nothing implements would otherwise leave every other case green.

## 29 mutants, each RUN — and the two that SURVIVED are the record

| mutant | outcome |
|---|---|
| M1 the `hostname_verifier` row removed | 19 fail |
| M2 the `trust_manager` row removed | 9 |
| M3 the interface retargeted to `Runnable` | 8 |
| M4 any constant accepted, not just 1 | 3 |
| M5 the returned register need not be the loaded one | 1 |
| **M6 `len(body) == 2` -> `>= 2` (a PREFIX match)** | **30 passed — SURVIVED** |
| M7 trust manager accepts any body ENDING in `return-void` | 1 |
| M18 trust manager accepts any body STARTING with `return-void` | 1 |
| M8 an unreadable body treated as empty | 1 |
| M9 an unknown kind answers `not_proven` instead of raising | 1 |
| M10 `with_xref` ignored in the module | 3 |
| M11 the adapter accepts `with_xref` and drops it | 1 |
| M12 `permissive_count` counts every row | 1 |
| M13 `constructed_in` always empty | 3 |
| M14 rows not sorted | 3 |
| M15 only permissive rows reported | 9 |
| M16 `const/16` dropped from the accepted forms | 1 |
| M17 `checkClientTrusted` added to the table | 3 |
| M19 duplicate declarations not deduplicated | 1 |
| M20 the duck-typed-sibling test removed | 5 |
| M21 the superclass test removed | 3 |
| M22 the whole class-level block removed | 8 |
| M23 the interface skip removed | 3 |
| M24 the block applied to `hostname_verifier` too | 6 |
| **M25 the block overwrites a `not_proven` reason too** | **45 passed — SURVIVED** |
| M26 the MCP payload drops `reason` | 1 |
| M27 the MCP schema default flipped | 1 |
| M28 `len(body) <= 2` (raises IndexError on a 1-instruction body) | 1 |
| M29 `_HEADER` widened to any dot/hash line | 2 |

**M6 is OUTPUT-EQUIVALENT, and the first explanation of why it is not was
mechanically wrong.** Execution starts at offset 0, so if the first two
instructions are `const 1; return`, what follows is dead and the method IS
permissive — which is why the relaxation passed all 30 cases. The first cut
claimed a leading PAYLOAD (dexllm#75) would render a fabricated prefix; a
correctness reviewer measured that `GetWidthFromBytecode` consumes a payload as
one unit-run and `DecodeInstruction` gives opcode 0x00, so it renders `0x0: nop`,
which can never be the `const/4 v1, #1` a prefix match needs. Whole-body equality
is kept as a deliberate CONSERVATISM — it accounts for every instruction instead
of resting on a reachability argument, and a 2-instruction body is exactly 2 code
units, so nothing can precede or follow it — and the `trailing` case pins that
CHOICE and says what it costs (a permissive `verify` with trailing dead code is
missed). M18 is its trust-manager twin, which the harness surfaced next.

**M25 is the second survivor and a real information loss.** Applying the
class-level block unconditionally overwrites the BODY's reason: `LBaseTrust;` is
blocked (it declares a 3-argument sibling) AND its 2-argument method has no body
at all, and the body's reason is the specific one. `reason` is the field that
makes a row actionable, so the block now explains only why a PERMISSIVE body was
not enough — pinned by a case that did not exist until the harness found this.

The harness snapshots the three production files, asserts a manifest before every
mutant AND after the run, and reports an anchor that does not match exactly once as
**NOT A MUTANT** rather than skipping it — a replacement that does not apply is not
a kill [[mutation-harness-restore-pitfalls]].

## What the two reviewers found — 1 FALSE POSITIVE, and 13 more, every one fixed here

Both ran in isolated COPIES with their own import shim, and both re-derived the
corpus census independently (0 `HostnameVerifier` implementors, 1
`X509TrustManager` — `InterfaceCls.dex`, permissive, `summarize_capabilities` `{}`
on it). Neither could break the pure `verify` predicate, the smali-format
assumption, the const-form completeness, the line-splitting, or the "no cap"
decision: the adversarial pass measured 0.02 ms per implementor without xref and
0.2 ms with (extrapolating to the 65,535 type-id ceiling ~13 s), a 46-source
subprocess sweep judged by EXIT STATUS gave `rc=0` everywhere with no hang, and
`_constructed_in`'s `not in seen` was measured genuinely O(n^2) (0.46 s -> 1.89 s
for 16k -> 32k distinct callers) yet 1.9 s for 32,000 constructing methods of ONE
class, which is not a DoS.

**The finding that mattered is a FALSE POSITIVE** (above): an empty 2-argument
`checkServerTrusted` beside a 3-argument sibling that pins a hostname, reported
`permissive`. Built with plain `javac` + `d8`, cited to Conscrypt's own
duck-typing dispatcher, and it made the tool accuse a correct app.

**The second is the refutation of "strictly stronger"** (above), and with it the
docstring's central claim — *"the object itself always has one — it MUST implement
`javax.net.ssl`'s own interface"* — which is true of the Java TYPE and false of
the PREDICATE, since `find_classes_implementing` sees only a DECLARATION.

**And a third the reviewers could not have caught, because I found it first by
reading my own diff:** `find_classes_implementing` answers per DECLARATION, so a
multidex or packer session reported **8 rows for 4 components** and an MCP `count`
of 8. Rows are deduplicated by descriptor now, and the residual — which
declaration's body is judged — is the library-wide first-wins resolution,
DOCUMENTED and pinned in both directions (`add_dumped_dexes(prefer=True)` puts
the dump first, so the unpacked body is the one judged). Both reviewers reached
the same shape independently, one of them through the documented
`add_dumped_dexes` workflow.

### The rest were guards that passed having exercised nothing

**Ten mutants survived the first guard file, and the fixture is why.** Its
components had exactly one construction site, no caller of a non-constructor
method, one row per class, and no duck-typed overload — the precise conditions
under which each of these is EQUIVALENT:

| surviving mutant | what the fixture could not distinguish |
|---|---|
| the `-><init>(` filter removed | no caller reached a non-constructor method (measured: differs on **757 of 1,500** classes of `app-prod-debug.apk`, changing the field's meaning from "constructed in" to "referenced in") |
| the caller dedupe removed | nothing constructed a component twice (differs on 40 of 1,500) |
| `sorted(seen)` -> `seen` | one caller, so both orders coincide |
| the row sort's second component dropped | one row per class, and `sorted` is stable |
| 4 of the 7 MCP payload fields dropped | the SDK layer had a full-field guard and the MCP layer had none — `reason` is what makes a `permissive` verdict actionable |
| `_HEADER` widened to `\..*` / `#.*` | the unreadable-body case starts with a LETTER, so neither widening was tested |
| `len(body) == 2` -> `<= 2` | **raises IndexError** on a 1-instruction body, which `while (true) {}` compiles to |
| the MCP schema `default` flipped | dexllm#49 pinned exactly this for `app_only` and it was not generalised |

Every one is closed, mostly by REBUILDING THE FIXTURE — which is the advantage of
having authored it. It now carries `PermissiveBoth` (both interfaces, the only
two-row class), a verifier with a non-constructor method and TWO constructors
whose callers arrive unsorted, `DuckTrust` / `ExtTrust` / `InheritTrust` (the
three duck-typing traps, the last reachable ONLY through the superclass test),
and `SubVerifier` / `SubAllowAll` (the sub-interface and its invisible
implementor, whose body is exactly the shape the detector proves).

### Two claims of mine were mechanically wrong

The `trailing` guard justified whole-body equality with *"a method whose first
code units are a PAYLOAD renders a fabricated prefix"*. **False**:
`GetWidthFromBytecode` consumes a payload as one unit-run and `DecodeInstruction`
gives opcode 0x00, so a leading payload renders `0x0: nop` — which can never be
the `const/4 v1, #1` a prefix match needs. A prefix match IS sound on a body
rendered from offset 0, so the mutant is **output-equivalent** and whole-body
equality is a deliberate conservatism, not a soundness property. The guard says so
now, and the real trade-off — a permissive `verify` with trailing dead code is
missed — is stated instead of hidden behind a wrong reason.

And the new cross-file doc link `api.md#8-c2--ioc-extraction-python` was DANGLING
(the heading is `## 8. IOC extraction (Python)`). `test_no_in_page_doc_link_is_dangling`
matches a same-page target only, so cross-file anchors were unguarded, and
the audit found **two more, both pre-existing**: `api.md` -> a `usage.md` heading
whose slug drops a backticked `<init>`, and `usage.md` -> a CLAUDE.md heading that
had been retitled. All three fixed, and
`test_no_cross_file_doc_anchor_is_dangling` closes the class with the SAME slug
...
/ `FieldRead` in `resolve_call_args`, `hostname_verifier` / `trust_manager` here —
which the review flagged as the dexllm#38 / dexllm#64 shape. It is not: those were
one NAME for two artifacts a consumer joins (`dex_count`'s two scopes; `api` as a
Dalvik descriptor in one tool and a dotted AOSP signature in another). A `kind`
discriminator is the SAME concept in both records — "which variety of this record
is it" — with a vocabulary scoped to its record, which is what a discriminator is
for. Recorded rather than renamed, so the judgement is visible.

## A measurement of mine was invalidated by my own harness

A full suite was running while I swapped `src/dexllm` to HEAD for the OFF capture,
and `test_every_mcp_tool_name_exists_on_another_layer` failed: it calls
`inspect.getsource(impl)`, which re-READS the file from disk, and at that moment the
file was HEAD's. The run reported **1 failed / 1228 passed** and the failure was an
artefact of the swap, not of the change [[verify-build-identity-before-measuring]].
Discarded and re-run clean (1230 / 24 / 0). **Nothing may touch the tree while a
suite runs** — the same shape as the concurrent-reviewer trap this file already
