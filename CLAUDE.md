...
**Malformed-dex bounds (partial — full crash-proofing needs process isolation; see below):** raw reads on a crafted/truncated `.dex` are bounded so the reviewed crash sites can't OOB-read. Builder ([method_snapshot_builder.cpp](native/dad_cpp/method_snapshot_builder.cpp)): `insns_size` is clamped to the dex image (a garbage code-item header can't make the decode loop march off the mmap); `SafeWidth()` guards the size-field read `dex::GetWidthFromBytecode` does for payload markers before calling it; switch/fill-array payload tails clamp to `insns_size`; the exception-table leb128 walk is bounded by `IDexCodeSource::GetDexImageRange(dex_id)` → the mmap `[begin,end)` (default `{null,null}`; `DexItemCodeSource` returns the real range, `MockCodeSource` uses the default — test data is well-formed). Metadata ([dexitem_code_source.cpp](native/core_ext/dexitem_code_source.cpp)): every secondary table index (name_idx/type_idx/proto_idx/class_idx) goes through `SafeAt()` (bounds-checked) instead of raw `vec[idx]`; `TypeList.size` clamps to the mmap; class-data / static-value scan windows clamp to the mmap end. DAD-IR ([instruction.cpp](native/dad_cpp/instruction.cpp)): IR constructors throw on a null operand where DAD raises `AttributeError` (e.g. `MoveExpression` on move-result-without-invoke) so the per-method catch skips it instead of segfaulting.

**Known gap — broad hardening (B, in progress):** a 30-byte random-corruption fuzz still segfaults on most variants. The above fixed whole crash *classes* (parity 25/25, well-formed corpus unaffected) but the surface is vast — remaining segfaults live across the DAD-IR port (more null/index assumptions) and the **vendored slicer's own decode** (not ours to rewrite). Exhaustive internal hardening is impractical and partly conflicts with the `// DAD:` 1:1 port. The bounded, complete solution for processing untrusted dex is **process isolation** (run decompile in a disposable worker; a crash is contained, the analyzer survives) — `safe.py` currently does *thread* isolation (catches hangs, NOT segfaults). TODO: add a process-isolated `safe_decompile_*` worker.
