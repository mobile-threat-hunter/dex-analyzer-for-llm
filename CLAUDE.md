...
**Follow-up — escape the full Cc control range, not just C0 (2026-06-20):** `AppendUtf16Escaped` originally escaped only C0 controls (`< 0x20`) + surrogates, emitting **C1 controls (0x80–0x9F) and DEL (0x7F) as raw UTF-8**. For a binary blob stored as a string — e.g. Play-services' embedded DER platform certificate (`Lcom/google/android/gms/common/zzi;->zzb()[B` in Telegram: a 1196-byte X.509 cert with `android@android.com`) — the 105 C1 bytes (incl. the RSA OID's `0x86`s) render **invisible**, making the literal look truncated/garbled even though it round-trips byte-for-byte. Fixed to escape every Unicode **control char (category Cc: C0 `0x00–0x1F`, DEL `0x7F`, C1 `0x80–0x9F`)** + surrogates as `\uXXXX`; printable code units (letters, `¨/Õ/÷`, CJK, 한글) still pass as readable UTF-8, output stays valid Java and round-trips identically. Verified on the Telegram cert: C1-raw 105 → 0, the OID now shows `*H÷`; parity 28/28 (mutf8 suite gains C1/DEL/printable-Latin-1 cases), sweep 0-crash/25,309-class.
...
| `list_strings()` → list[str] | Every distinct string literal across all loaded dexes (MUTF-8→UTF-8, deduplicated). Thin binding over `DexItem::GetStrings()`. Foundation for static IOC/C2 extraction. |
| `dexllm.extract_iocs(dk, …)` → dict | **Python** ([ioc.py](src/dexllm/ioc.py)). Static C2/network-IOC extraction: classifies `list_strings()` into `{urls, ips, domains, emails, onion}` (denoising framework package names that look like hosts), and cross-refs each indicator to its referencing method via L7 — VirusTotal's contacted-addresses view, recovered statically + with code locations. Exposed as the `extract_iocs` MCP tool. |
| `dexllm.dangerous_permission_apis(dk)` / `dangerous_permission_api_callers(dk)` → dict | **Python** ([dangerous_api.py](src/dexllm/dangerous_api.py)). Joins AOSP's `@RequiresPermission` permission→API map ([aosp_data_set](https://github.com/mobile-threat-hunter/aosp_data_set), dangerous slice bundled at [data/dangerous_perm_api.json](src/dexllm/data/dangerous_perm_api.json)) against the APK's external method refs → which **dangerous** permissions are exercised through real API calls, and `dangerous_permission_api_callers` adds the calling methods (default `app_only=True` drops bundled framework/official-library callers — androidx/kotlin/play-services — so you see the app's own usage; `app_only=False` keeps them). `dataset_path=`/`$DEXLLM_AOSP_DATASET` override the bundled table. MCP tools `dangerous_permission_apis` / `dangerous_permission_api_callers`. |
