...
### Production fix — `return 0`/`1` in a `boolean` method → `return false`/`true` (2026-06-18)

A `Z`-returning method that returns an integer constant emitted `return 0;` / `return 1;` — **uncompilable** Java (no int→boolean coercion). In Dalvik a boolean IS an int 0/1, and `const/4` carries no boolean type, so the returned `Constant` is typed `"I"` and routes to `visit_constant_int` → the bare number. DAD emits the int verbatim (its own bug — reproduced on `JobSchedulerServiceV.a()Z` → `return 0;`). **Fix:** in `visit_return` ([writer.cpp](native/dad_cpp/writer.cpp)), when `snap.meta.ret_type == "Z"` and the operand is an integer `Constant` with value 0/1, emit `false`/`true` (only 0/1 remapped; other values — impossible for a valid Z return — left untouched; non-constant returns fall through to normal emission). Beyond-DAD divergence, same precedent as the catch-clamp (no `*DADFaithful` sibling — parity suites don't assert this; e2e *improves* where DAD was invalid). parity 26/26, sweep 0-crash. tvleanback byte-match-vs-DAD dips 98.0%→97.2% (4 methods now emit valid `return false`/`true` where DAD emits invalid `return 0`/`1` — a metric artifact of being more correct than the reference, not a regression).
