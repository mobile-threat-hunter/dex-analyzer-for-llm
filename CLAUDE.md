...
**Measured (apples-to-apples, tvleanback):** empty `while ()` **104 → 17**; parity 26/26, sweep 0-crash/159,305-method unchanged; tvleanback 500-sample exact-match flat at 97.0% (fixed loops go from catastrophically-empty to body-present but still differ from DAD on inner-`if`/`break` structuring — a separate, smaller residual). Remaining empty `while()`/`if()` are other sub-causes (not the statement-header wrapping case).
