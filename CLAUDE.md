...
**Malformed-dex bounds (partial — see "broad hardening" below):** the builder's raw reads over `insns[]`/payloads/exception-tables are bounded against the dex image so a crafted/truncated `.dex` can't OOB-read inside the reviewed sites: switch/fill-array payload tails clamp to `insns_size`; the decode loop rejects an instruction that extends past `insns`; the exception-table leb128 walk is bounded by `IDexCodeSource::GetDexImageRange(dex_id)` → the mmap `[begin,end)` (default `{nullptr,nullptr}`; `DexItemCodeSource` returns the real range, `MockCodeSource` uses the default since test data is well-formed). `core_ext/dexitem_code_source.cpp` similarly clamps its class-data / static-value scan windows to the mmap end. NOTE: a 30-byte random-corruption fuzz still segfaults elsewhere (vendored slicer decode, metadata reads, races on corrupt data) — full "crash-proof on any malformed dex" is a larger, separate effort.
