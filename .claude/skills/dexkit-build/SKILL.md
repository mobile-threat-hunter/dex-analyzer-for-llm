...
- Build artifacts: `<repo>/build/cp313-cp313-linux_x86_64`
...
```bash
R="$(git rev-parse --show-toplevel)" && cd "$R/build/cp313-cp313-linux_x86_64" && ninja 2>&1 | tail -5 && cd "$R" && pip install -e . --no-build-isolation 2>&1 | tail -3
```
