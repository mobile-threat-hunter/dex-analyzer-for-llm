...
// dexllm: two includes upstream does not have. <algorithm> is live (std::sort
// in InitBaseCache); <unordered_set> is NOT -- it has no use in this file and
// had none before dexllm#80 either (0 uses in the span that moved out), so it
// is pre-existing dead weight rather than a leftover of that reduction, and
// removing it is a separate change. Until dexllm#80 this hunk merged with the
// mutf8 include below it at three lines of context and inherited ITS marker;
// retiring that include left this one standing unmarked, which is the
// hunk-granularity limit docs/dexkit-vendor-divergences.md states.
#include <algorithm>
...
// dexllm#80: the baksmali-style smali renderer (11 file-local helpers plus
// RenderMethodSmali / RenderClassSmali) and EnumerateInvokeSites used to live
// here -- 560 lines, about 44% of everything this fork added, and none of it
// upstream code under any name. They moved to native/core_ext/smali_render.cpp
// and native/core_ext/invoke_args.cpp; the whole input was nine PUBLIC
// accessors. Same direction as dexllm#32, which moved the L4 argument-origin
// analysis for the same reason: a dexllm analysis outside vendor/ can be
// tested, reviewed and rebased without colliding with upstream.
