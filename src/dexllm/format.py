...
if TYPE_CHECKING:
    from ._dexkit_core import ClassSummary, DexKit
...
def _class_keyword(flags: int) -> str:
    if flags & _ACC_ANNOTATION:
        return "@interface"
    if flags & _ACC_ENUM:
        return "enum"
    if flags & _ACC_INTERFACE:
        return "interface"
    return "class"


def format_class_summary(summary: "ClassSummary", *, indent: str = "    ") -> str:
    ...
    if summary.is_internal:
        ...
        out.append("// EXTERNAL — referenced but not defined in any loaded dex")
    ...
    if summary.fields:
        ...
        for f in summary.fields:
            out.append(f"{indent}{_d.descriptor_to_java(f.type)} {f.name};")
        out.append("")
