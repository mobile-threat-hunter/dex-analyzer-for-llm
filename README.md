...
| Permissive TLS | `detect_permissive_tls()` — the `javax.net.ssl` `HostnameVerifier` / `X509TrustManager` implementations the app declares, each with a PROVEN verdict: `permissive` means the body was proven to accept every hostname or every certificate chain. Names the INTERFACE rather than the call that installs it, so the OkHttp / Volley / Retrofit paths — which install through bundled library code with no framework spelling — are caught too (dexllm#53). `not_proven` is the absence of a proof, not a clean bill of health — and a trust manager whose 2-arg `checkServerTrusted` is not the overload Conscrypt duck-types is DECLINED rather than accused |
...
[^handle]: Both sections' **extent** is bounded and their **alignment** is
    ...
    goes BEYOND ART instead, and it rejects (almost) nothing: a per-image memo
    plus an entry budget, so a method_handle section shared between the slices of
    a v41 container is walked once rather than once per sibling — which is what
    stops that walk being quadratic in the container (B2d).
