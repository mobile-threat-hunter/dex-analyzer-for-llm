...
| Permissions | `dangerous_permission_apis()` / `_callers()` — which dangerous permissions the APK exercises through real framework API calls (AOSP `@RequiresPermission` map), and the methods that call them |
