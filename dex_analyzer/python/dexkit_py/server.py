...
class Session:
    ...
    def __init__(self, sid: str, apk_path: str, tmpdir: str, dk: DexKit):
        self.id = sid
...
async def _run_agent(sess: Session, prompt: str):
    """Generator yielding SSE events for one user prompt.
    ...
    for turn in range(MAX_TURNS):
        try:
            ...
            yield _sse("error", {"where": "messages.create", "error": f"{type(e).__name__}: {e}"})
            return
        ...
        for block in resp.content:
            ...
            if btype == "text":
                ...
                yield _sse("thinking", {"turn": turn, "text": getattr(block, "thinking", "")})
            elif btype == "tool_use":
                yield _sse("tool_use", {
                    "turn": turn,
                    "id": block.id,
...
@app.post("/analyze")
async def analyze(session_id: str = Form(...), prompt: str = Form(...)) -> EventSourceResponse:
...
@app.get("/tools")
def list_tools() -> dict:
    """Return the tool catalog served to the model. Useful for the UI to
    render a 'what can Claude do here' panel."""
