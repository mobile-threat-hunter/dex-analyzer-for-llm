...
def _paginate(items: list, offset: int = 0, limit: int = DEFAULT_LIST_LIMIT) -> dict:
    """Standard list-response shape for tools."""
    total = len(items)
...
def _truncate(text: str, max_chars: int) -> dict:
    """Standard text-response shape for decompile/render tools."""
    full = len(text)
    if full <= max_chars:
...
def _t_list_classes(dk, pattern: str | None = None, offset: int = 0,
                    limit: int = DEFAULT_LIST_LIMIT) -> dict:
    items = _filter_pattern(dk.list_classes(), pattern)
    return _paginate(items, offset, limit)


def _t_list_class_methods(dk, class_descriptor: str) -> dict:
    return {"class": class_descriptor, "methods": dk.list_class_methods(class_descriptor)}


def _t_decompile_method(dk, method_descriptor: str,
                        max_chars: int = DEFAULT_DECOMPILE_CHARS) -> dict:
...
def _t_decompile_class(dk, class_descriptor: str,
                       max_chars: int = DEFAULT_CLASS_CHARS) -> dict:
...
def _t_find_classes_by_name(dk, name: str, match_type: str = "contains",
                            ignore_case: bool = False, limit: int = 50) -> dict:
...
def _t_find_classes_by_super(dk, super_class: str, match_type: str = "equals",
                             limit: int = 50) -> dict:
    hits = dk.find_classes_by_super(super_class, match_type=match_type)
    return _paginate([_match_to_desc(h) for h in hits], 0, limit)


def _t_find_classes_implementing(dk, interface_class: str, match_type: str = "equals",
                                  limit: int = 50) -> dict:
    hits = dk.find_classes_implementing(interface_class, match_type=match_type)
    return _paginate([_match_to_desc(h) for h in hits], 0, limit)


def _t_find_classes_by_annotation(dk, annotation_class: str, match_type: str = "equals",
                                   limit: int = 50) -> dict:
    hits = dk.find_classes_by_annotation(annotation_class, match_type=match_type)
    return _paginate([_match_to_desc(h) for h in hits], 0, limit)


def _t_find_classes_using_strings(dk, strings: list[str], match_type: str = "contains",
                                   ignore_case: bool = False, limit: int = 50) -> dict:
    hits = dk.find_classes_using_strings(strings, match_type=match_type,
                                         ignore_case=ignore_case)
    return _paginate([_match_to_desc(h) for h in hits], 0, limit)


def _t_find_methods_by_name(dk, name: str, match_type: str = "contains",
                             declaring_class: str = "", ignore_case: bool = False,
                             limit: int = 50) -> dict:
    hits = dk.find_methods_by_name(name, match_type=match_type,
                                   declaring_class=declaring_class,
                                   ignore_case=ignore_case)
    return _paginate([_match_to_desc(h) for h in hits], 0, limit)


def _t_find_methods_using_strings(dk, strings: list[str], match_type: str = "contains",
                                   ignore_case: bool = False, limit: int = 50) -> dict:
    hits = dk.find_methods_using_strings(strings, match_type=match_type,
                                         ignore_case=ignore_case)
    return _paginate([_match_to_desc(h) for h in hits], 0, limit)


def _t_find_call_sites_to_api(dk, api_descriptor: str, limit: int = 50) -> dict:
...
def _t_get_class_summary(dk, class_descriptor: str) -> dict:
...
def _t_render_method_smali(dk, method_descriptor: str,
                            max_chars: int = DEFAULT_DECOMPILE_CHARS) -> dict:
    out = dk.render_method_smali(method_descriptor)
    return {"descriptor": method_descriptor, **_truncate(out, max_chars)}


def _t_capability_report(dk, max_chars: int = DEFAULT_CLASS_CHARS) -> dict:
    from .capability import summarize_capabilities
    rep = summarize_capabilities(dk)
    # CapabilityReport is dataclass-like; coerce.
    if hasattr(rep, "__dict__"):
        d = {k: v for k, v in rep.__dict__.items() if not k.startswith("_")}
    else:
        d = dict(rep)
    return d
...
TOOL_DEFINITIONS: list[dict] = [
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "pattern": {"type": "string", "description": "optional regex to filter descriptors"},
                "offset": {"type": "integer", "default": 0},
                "limit": {"type": "integer", "default": DEFAULT_LIST_LIMIT, "maximum": 1000},
            },
        },
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "class_descriptor": {"type": "string", "description": "e.g. 'Lcom/foo/Bar;'"},
            },
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "name": {"type": "string"},
                "match_type": {"type": "string", "enum": ["equals", "contains", "starts_with", "ends_with", "regex"], "default": "contains"},
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "super_class": {"type": "string", "description": "e.g. 'Landroid/app/Activity;'"},
...
def execute(name: str, args: dict, dk) -> dict:
...
def tool_definitions() -> list[dict]:
    """Return the tool catalog. The Anthropic Messages API accepts this
    directly as the `tools=` argument; MCP serves the same entries via
    the `list_tools` protocol method."""
